...
export function addColumn(tr: Transaction, { map, tableStart, table }: Rect, col) {
	...
	for (let row = 0; row < map.height; row++) {
		...
		if (col > 0 && col < map.width && map.map[index - 1] == map.map[index]) {
			...
			let type =
				refColumn == null ? tableNodeTypes(table.type.schema).tablecell : table.nodeAt(map.map[index + refColumn]).type;
...
export function removeColumn(tr: Transaction, { map, table, tableStart }: Rect, col) {
...
export function addRow(tr: Transaction, { map, tableStart, table }: Rect, row) {
	...
	for (let col = 0, index = map.width * row; col < map.width; col++, index++) {
		...
		if (row > 0 && row < map.height && map.map[index] == map.map[index - map.width]) {
			...
			let type =
				refRow == null
					? tableNodeTypes(table.type.schema).tablecell
					: table.nodeAt(map.map[index + refRow * map.width]).type;
			cells.push(type.createAndFill());
		}
	}
	tr.insert(rowPos, tableNodeTypes(table.type.schema).tablerow.create(null, cells));
	return tr;
}
...
export function removeRow(tr: Transaction, { map, table, tableStart }: Rect, row) {
	let rowPos = 0;
	for (let i = 0; i < row; i++) rowPos += table.child(i).nodeSize;
...
export const splitCell: Command = (state, dispatch) => {
	...
	return splitCellWithType(({ node }) => nodeTypes[node.type.spec.tableRole])(state, dispatch);
};

// :: (getCellType: ({ row: number, col: number, node: Node}) → NodeType) → (EditorState, dispatch: ?(tr: Transaction)) → bool
// Split a selected cell, whose rowpan or colspan is greater than one,
// into smaller cells with the cell type (th, td) returned by getType function.
export function splitCellWithType(getCellType): Command {
	return (state, dispatch) => {
		...
		if (dispatch) {
			...
			for (let i = 0; i < rect.right - rect.left; i++)
				attrs.push(
					colwidth ? setAttr(baseAttrs, 'colwidth', colwidth && colwidth[i] ? [colwidth[i]] : null) : baseAttrs
				);
			let lastCell;
			for (let row = rect.top; row < rect.bottom; row++) {
...
export function setCellAttr(name, value): Command {
...
function deprecated_toggleHeader(type): Command {
	return function (state, dispatch) {
		if (!isInTable(state)) return false;
		if (dispatch) {
...
function findNextCell($cell: ResolvedPos, dir: -1 | 1) {
	if (dir < 0) {
...
export function createTable({ rows = 3, cols = 3, header = true } = {}): Command {
	return function (state, dispatch) {
		const tableTypes = tableNodeTypes(state.schema);
		const cells = [],
			cellType = tableTypes.tablecell;
		const headers = [],
			headerType = tableTypes.tableheader;
		for (var i = 0; i < cols; i++) {
			cells.push(cellType.createAndFill());
			if (header) {
				headers.push(headerType.createAndFill());
			}
		}
		const nodes = [],
			rowType = tableTypes.tablerow;
		for (var i = 0; i < rows; i++) {
			nodes.push(rowType.createAndFill(null, header && i === 0 ? headers : cells));
		}
		if (dispatch) {
			dispatch(state.tr.replaceSelectionWith(tableTypes.table.createAndFill(null, nodes)));
		}
		return true;
	};
}
