...
import { cellAround, pointsAtCell, setAttr } from './util.js';

export const ColumnResizingKey = new PluginKey('TableColumnResizing');

export function columnResizing({ handleWidth = 5, cellMinWidth = 25, lastColumnResizable = true } = {}) {
	let plugin = new Plugin({
		...
		state: {
			...
			apply(tr, pluginState: ResizeState) {
		...
		props: {
			attributes(state) {
				...
				return pluginState.activeHandle > -1 ? { class: 'ProseMirror-resize-cursor' } : null;
			},
			handleDOMEvents: {
...
class ResizeState {
	...
	apply(tr: Transaction) {
		let meta = tr.getMeta(ColumnResizingKey);
		if (meta && meta.setHandle != null) return new ResizeState(meta.setHandle, null);
		if (meta && meta.setDragging !== undefined) return new ResizeState(this.activeHandle, meta.setDragging);
...
function handleMouseMove(view, event, handleWidth, cellMinWidth, lastColumnResizable) {
	let pluginState = ColumnResizingKey.getState(view.state);
...
function handleMouseDown(view, event, cellMinWidth) {
	...
	view.dispatch(
		view.state.tr.setMeta(ColumnResizingKey, {
...
function updateColumnWidth(view, cell, width) {
	...
	for (let row = 0; row < map.height; row++) {
		...
		let colwidth = attrs.colwidth ? attrs.colwidth.slice() : Array(attrs.colspan).fill(0);
...
function displayColumnWidth(view, cell, width, cellMinWidth) {
	...
	dom.nodeView.updateColumns(cellMinWidth, col, width);
	dom.nodeView.drawCellSelection();
}
...
function handleDecorations(state, cell) {
	...
	for (let row = 0; row < map.height; row++) {
		...
		if (
			...
			dom.className = 'ProseMirror-column-resize-handle';
