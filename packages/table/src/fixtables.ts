...
/**
 * "So the approach in the new module, where row- and column-spanning
 *  cells were a requirement, is to define a much simpler schema, which
 *  only says that table cells go in rows and rows in tables, and to
 *  enforce table shape constraints in the appendTransaction method of
 *  a table-editing plugin. This means that the document may, for some
 *  editing operations, end up with non-rectangular tables at the end of
 *  a transaction, but another transaction fixing that will be appended
 *  right away, before the content is even drawn to the screen."
 * https://discuss.prosemirror.net/t/rfc-deprecating-prosemirror-schema-table-and-attribute-constraints-in-schemas/864
 */
