...
export class CellSelection extends Selection {
	...
	map(doc, mapping) {
		...
		if (pointsAtCell($anchorCell) && pointsAtCell($headCell) && inSameTable($anchorCell, $headCell)) {
			...
			if (tableChanged && this.isColSelection) return CellSelection.colSelection($anchorCell, $headCell);
	...
	forEachCell(f: (node: PMNode, pos: number) => void) {
		let table = this.$anchorCell.node(-1),
			map = TableMap.get(table),
			start = this.$anchorCell.start(-1);
...
class CellBookmark {
	...
	resolve(doc) {
		...
		if (
			$anchorCell.parent.type.spec.tableRole == 'tablerow' &&
			$headCell.parent.type.spec.tableRole == 'tablerow' &&
...
export function drawCellSelection(state: EditorState): DecorationSet {
...
export function normalizeSelection(state: EditorState, tr, allowTableNodeSelection) {
	let sel = (tr || state).selection,
		doc = (tr || state).doc,
		normalize,
		role;
	if (sel instanceof NodeSelection && (role = sel.node.type.spec.tableRole)) {
		if (role === 'tablecell' || role === 'tableheader') {
