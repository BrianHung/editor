...
export function pastedCells(slice: Slice) {
	...
	if (role == 'row') {
		for (let i = 0; i < content.childCount; i++) {
			...
			if (left || right) cells = fitSlice(tableNodeTypes(schema).tablerow, new Slice(cells, left, right)).content;
			rows.push(cells);
		}
	} else if (role == 'cell' || role == 'tableheader') {
		rows.push(
			openStart || openEnd
				? fitSlice(tableNodeTypes(schema).tablerow, new Slice(content, openStart, openEnd)).content
				: content
		);
	} else {
		return null;
...
function ensureRectangular(schema, rows) {
	...
	for (let r = 0; r < widths.length; r++) {
		...
		if (widths[r] < width) {
			let empty = tableNodeTypes(schema).tablecell.createAndFill(),
				cells = [];
...
export function fitSlice(nodeType, slice: Slice) {
...
function growTable(tr, map, table, start, width, height, mapFrom) {
	...
	if (width > map.width) {
		for (let row = 0, rowEnd = 0; row < map.height; row++) {
			...
			else add = emptyHead || (emptyHead = types.tableheader.createAndFill());
	...
	if (height > map.height) {
		...
		for (let i = 0, start = (map.height - 1) * map.width; i < Math.max(map.width, width); i++) {
			let header = i >= map.width ? false : table.nodeAt(map.map[start + i]).type == types.tableheader;
			cells.push(
				header
		...
		let emptyRow = types.tablerow.create(null, Fragment.from(cells)),
			rows = [];
