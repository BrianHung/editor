...
export class SyntaxHighlightView {
	...
	async importLanguages(langs: LanguageDescription[]) {
		console.log('hello worimportLanguagesld');
		await Promise.all(
			langs
				.map(language => language.load())
				.concat(
					// @ts-ignore
					syntaxHighlight
						? []
		...
		// Dispatch transaction to re-trigger syntax highlighting after imports resolve.
...
export class SyntaxHighlightState {
	...
	applyTransaction(tr: Transaction) {
		...
		if (imported) {
			...
			let modified = this.codeblocks
				.map(pos => ({ node: tr.doc.resolve(pos + 1).parent, pos }))
			...
			this.codeblocks = mappedPos.reduce(
				(codeblocks, { deleted, pos }) => (deleted ? codeblocks : codeblocks.concat([pos])),
				[]
			);

			// Remove decorations that correspond to deleted codeblocks.
			const deleted = mappedPos.reduce(
				(codeblocks, { deleted, pos }) => (deleted ? codeblocks.concat([pos]) : codeblocks),
				[]
			);
			const removed = deleted
				.map(pos =>
					this.decorations.find(pos, pos - tr.doc.resolve(pos).parentOffset + tr.doc.resolve(pos).parent.nodeSize)
				)
				.flat();
				...
				if (lang.support) {
					...
					decorations.push(
						Decoration.node(pos, pos + node.nodeSize, {
							class: `language-${lang.name.toLowerCase()}`,
						})
					);
					let startPos = pos + 1;
					syntaxHighlight(
