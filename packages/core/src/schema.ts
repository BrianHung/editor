...
export function schemaFromExtensions(extensions: Extensions, topNode?: SchemaSpec['topNode']): Schema {
	...
	const schemaAttrs = schemaAttrsFromExtensions(extensions);
	for (const [type, spec] of Object.entries(nodes)) {
		const extraAttrs = schemaAttrs[type];
		if (!extraAttrs) continue;
		spec.attrs = {
			...spec.attrs,
			...extraAttrs,
		};
 ...
 * @returns
 */
function schemaAttrsFromExtensions(extensions: Extensions): Record<string, SchemaAttributeSpec> {
	const attrs = Object.create(null);
	for (const { schemaAttrs } of extensions) {
