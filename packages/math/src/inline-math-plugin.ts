...
function modifiedTextblocks(tr: Transaction): { node: PMNode; pos: number }[] {
	...
	const reduce = (node, pos) => {
		node.isTextblock && !node.type.spec.code && textblocks.set(pos, { node, pos });
	...
	return new Plugin({
		...
		state: {
			...
			apply(tr, prevState) {
				...
				const oldDecorations = modified
					.map(nodePos => prevDecorations.find(nodePos.pos, nodePos.pos + nodePos.node.nodeSize))
					.flat();
		...
		appendTransaction(transactions, oldState, newState) {
			...
			const textInsert = transactions.some(({ steps }) =>
