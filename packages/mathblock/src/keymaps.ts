 ...
 */
export function deleteMathBlock(state: EditorState, dispatch, view: EditorView) {
	if (!isSelectionEntirelyInsideMathBlock(state)) return false;
	if (backspace(state, dispatch, view)) return true; // default backspace handler
	...
	dispatch(
		tr
			.replaceRangeWith(
				from,
				to,
				schema.nodes.paragraph.create(null, mathblock.textContent ? schema.text(mathblock.textContent) : null)
			)
