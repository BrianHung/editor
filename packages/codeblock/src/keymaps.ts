 ...
 * bitbucket.org/atlassian/.../editor/editor/src/plugins/code-block/
 */
...
export function deleteCodeBlock(state: EditorState, dispatch, view: EditorView) {
	...
	const from = $cursor.before($cursor.depth),
		to = $cursor.end($cursor.depth) + 1,
		codeblock = $cursor.node();
	...
	dispatch(
		tr
			.replaceRangeWith(
				from,
				to,
				schema.nodes.paragraph.create(null, codeblock.textContent ? schema.text(codeblock.textContent) : null)
			)
...
/**
 * Useful for deleting contents of node without deleting the node itself.
 * @param state
 * @param dispatch
 * @returns
 */
export const selectAllCodeBlock: Command = (state: EditorState, dispatch) => {
	if (!isSelectionEntirelyInsideCodeBlock(state)) return false;

	const { tr, selection: sel } = state;
	const $pos = sel.$from;

	// Use positions inside of the node.
	const start = $pos.start();
	const end = $pos.end();

	if (sel.from === start && sel.to === end) return false;
