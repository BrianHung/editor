 ...
 */
 export function deleteMathBlock(state: EditorState, dispatch, view: EditorView) {
  ...
  const from = $cursor.before($cursor.depth), 
    to = $cursor.end($cursor.depth) + 1, mathblock = $cursor.node();

  dispatch(
    tr.replaceRangeWith(from, to, schema.nodes.paragraph.create(null, mathblock.textContent ? schema.text(mathblock.textContent) : null))
