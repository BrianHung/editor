...
export default class CodeBlock extends Node {
  ...
  get schema(): NodeSpec {
    return {
      ...
      toDOM(node: PMNode) { 
        return ["pre", {class: "codeblock", ...node.attrs.lang && {"data-lang": node.attrs.lang}, ...node.attrs.lineNumbers && {"data-linenumbers": node.attrs.lineNumbers}}, 
  ...
  get plugins() {
    return [
      new Plugin({
        props: {
          // Detects if text copied is from VSCode and creates codeblock with language if selection is not currently in codeblock.
