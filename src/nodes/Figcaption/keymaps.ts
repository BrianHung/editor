...
function splitFigcaption(state: EditorState, dispatch) {
  const {$from, $to} = state.selection;
  if (state.selection instanceof NodeSelection) { return true; } // no-op
  if (dispatch) {
    ...
    if (!deflt.validContent(slice.content)) { return false; }
...
function joinMaybeClear(state, $pos, dispatch) {
  ...
  if (!before || !after || !before.lastChild.type.compatibleContent(after.type)) { return false; } // check node after is compatible with figcaption
  if (!$pos.parent.canReplace(index, index + 1) || !after.isTextblock) { return false; } // check if can replace node after and node after is textblock
  if (dispatch) {
    var tr = state.tr
