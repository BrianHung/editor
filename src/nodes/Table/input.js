...
function deleteCellSelection(state, dispatch) {
  ...
  if (dispatch) {
    let tr = state.tr, baseContent = tableNodeTypes(state.schema).tablecell.createAndFill().content
...
export function handlePaste(view, _, slice) {
  ...
  if (sel instanceof CellSelection) {
    if (!cells) cells = {width: 1, height: 1, rows: [Fragment.from(fitSlice(tableNodeTypes(view.state.schema).tablecell, slice))]}
