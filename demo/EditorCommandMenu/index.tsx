...
export const EditorCommandMenu = React.memo(() => {
	...
	useEditorEffect(
		function menuPosition(view) {
			console.log('view', view?.hasFocus());
			if (!view) return;
					...
					{menuState.items.length > 0 ? (
						<ul className="">
							{Object.entries(grouped).map(([group, items]) => (
								<div role="group" className="pb-1.5 pt-1 shadow-[rgba(55,55,55,0.10)_0px_-1px_0px]">
									<div className="px-3.5 py-1.5 text-xs font-medium text-gray-500" key={group} role="label">
...
const MenuItem = React.memo(function MenuItem({
	...
	const itemProps = {
		className: `flex w-[calc(100%-8px)] cursor-pointer space-x-2.5 px-2.5 py-1.5 transition-colors duration-[25ms] hover:bg-sky-100 aria-selected:bg-sky-200 aria-selected:active:bg-sky-300 aria-selected:hover:bg-sky-300/75 h-14 mx-1 rounded items-center`,
		role: 'option',
		onMouseDown,
