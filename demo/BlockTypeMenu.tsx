...
export const BlockTypeMenu = React.memo(props => {
	...
	const activeMenuItem = React.useMemo(() => {
		...
		const type = nodeTypeAtSelection(state.selection);
		return menuItems.find(item => item.nodeType === type);
	...
	return (
		...
		>
			...
			<Select.Portal>
				<Select.Content
					className="shadow-dropdown-menu select-none rounded-md bg-white text-sm"
					onCloseAutoFocus={onCloseAutoFocus}
					data-block-type-menu=""
				>
					<Select.ScrollUpButton className="flex items-center justify-center text-gray-400">
						<ChevronUpIcon />
					</Select.ScrollUpButton>
					<Select.Viewport>
						<Select.Group className="pb-1">
							<Select.Label className="flex h-8 items-center px-3 text-xs font-medium text-gray-500">
								Turn into
							</Select.Label>
							{menuItems.map(type => (
								<SelectItem key={type.title} value={type.title} {...type}>
									{type.title}
								</SelectItem>
							))}
						</Select.Group>
					</Select.Viewport>
					<Select.ScrollDownButton className="flex items-center justify-center text-gray-400">
						<ChevronDownIcon className="h-4 w-4" />
...
const SelectItem = React.memo(props => {
	return (
		<Select.Item
			className={
				'data-[highlighted,state="checked"]:bg-sky-300 mx-1 flex h-7 cursor-pointer items-center space-x-2 rounded px-3 transition-colors duration-[25ms] focus-visible:outline-0 data-[highlighted]:bg-sky-100 data-[state="checked"]:bg-sky-200'
			}
			value={props.value}
		>
