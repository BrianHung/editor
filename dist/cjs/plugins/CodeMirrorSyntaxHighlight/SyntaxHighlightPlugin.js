...
const findLanguage_1 = require("./findLanguage");
...
class SyntaxHighlightState {
    ...
    applyTransaction(tr) {
        ...
        if (imported) {
            let modified = this.codeblocks.map(pos => ({ node: tr.doc.resolve(pos + 1).parent, pos }))
                .filter(({ node }) => imported.includes(findLanguage_1.findLanguage(node.attrs.lang || defaultLang[node.type.name])));
            ...
            console.log("modified", modifiedCodeblocks, trRanges, tr);
    ...
    getDecorations(nodePos) {
        ...
        nodePos.forEach(({ node, pos }) => {
            let lang = findLanguage_1.findLanguage(node.attrs.lang || defaultLang[node.type.name]);
            if (lang) {
                if (lang.support) {
                    let startPos = pos + 1;
                    exports.syntaxHighlight(node.textContent, lang.support, ({ from, to, style }) => style && decorations.push(prosemirror_view_1.Decoration.inline(startPos + from, startPos + to, { class: style })));
