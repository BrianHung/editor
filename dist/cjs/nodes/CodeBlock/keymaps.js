...
exports.toggleLineNumbers = exports.deleteCodeBlock = exports.backspaceCodeBlock = exports.newLine = exports.lineUndent = exports.lineIndent = void 0;
...
function backspaceCodeBlock(state, dispatch, view) {
    if (!isSelectionEntirelyInsideCodeBlock(state))
        return false;
    return (0, prosemirror_commands_1.chainCommands)(prosemirror_commands_1.deleteSelection, deleteCodeBlock, prosemirror_commands_1.joinBackward, prosemirror_commands_1.selectNodeBackward)(state, dispatch, view);
}
exports.backspaceCodeBlock = backspaceCodeBlock;
function deleteCodeBlock(state, dispatch, view) {
    ...
    const from = $cursor.before($cursor.depth), to = $cursor.end($cursor.depth) + 1, codeblock = $cursor.node();
    dispatch(tr.replaceRangeWith(from, to, schema.nodes.paragraph.create(null, codeblock.textContent ? schema.text(codeblock.textContent) : null))
...
function toggleLineNumbers(state, dispatch) {
    ...
    if (dispatch) {
        ...
        dispatch(state.tr.setNodeMarkup(start, undefined, Object.assign(Object.assign({}, node.attrs), { lineNumbers: !node.attrs.lineNumbers })));
    }
    return true;
}
