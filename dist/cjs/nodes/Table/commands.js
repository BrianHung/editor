...
function selectedRect(state) {
    let sel = state.selection, $pos = util_1.selectionCell(state);
...
function addColumn(tr, { map, tableStart, table }, col) {
    ...
    if (util_1.columnIsHeader(map, table, col + refColumn))
        refColumn = col == 0 || col == map.width ? null : 0;
    for (let row = 0; row < map.height; row++) {
        let index = row * map.width + col;
        if (col > 0 && col < map.width && map.map[index - 1] == map.map[index]) {
            let pos = map.map[index], cell = table.nodeAt(pos);
            tr.setNodeMarkup(tr.mapping.map(tableStart + pos), null, util_1.addColSpan(cell.attrs, col - map.colCount(pos)));
            row += cell.attrs.rowspan - 1;
        }
        else {
            let type = refColumn == null ? schema_1.tableNodeTypes(table.type.schema).tablecell
...
function removeColumn(tr, { map, table, tableStart }, col) {
    ...
    for (let row = 0; row < map.height;) {
        ...
        if ((col > 0 && map.map[index - 1] == pos) || (col < map.width - 1 && map.map[index + 1] == pos)) {
            tr.setNodeMarkup(tr.mapping.slice(mapStart).map(tableStart + pos), null, util_1.removeColSpan(cell.attrs, col - map.colCount(pos)));
        }
        else {
...
function rowIsHeader(map, table, row) {
    let headerCell = schema_1.tableNodeTypes(table.type.schema).tableheader;
...
function addRow(tr, { map, tableStart, table }, row) {
    ...
    for (let col = 0, index = map.width * row; col < map.width; col++, index++) {
        if (row > 0 && row < map.height && map.map[index] == map.map[index - map.width]) {
            ...
            tr.setNodeMarkup(tableStart + pos, null, util_1.setAttr(attrs, "rowspan", attrs.rowspan + 1));
            col += attrs.colspan - 1;
        }
        else {
            let type = refRow == null ? schema_1.tableNodeTypes(table.type.schema).tablecell
                : table.nodeAt(map.map[index + refRow * map.width]).type;
            cells.push(type.createAndFill());
        }
    }
    tr.insert(rowPos, schema_1.tableNodeTypes(table.type.schema).row.create(null, cells));
    return tr;
}
...
function removeRow(tr, { map, table, tableStart }, row) {
    ...
    for (let col = 0, index = row * map.width; col < map.width; col++, index++) {
        ...
        if (row > 0 && pos == map.map[index - map.width]) {
            ...
            tr.setNodeMarkup(tr.mapping.slice(mapFrom).map(pos + tableStart), null, util_1.setAttr(attrs, "rowspan", attrs.rowspan - 1));
            col += attrs.colspan - 1;
        }
        else if (row < map.width && pos == map.map[index + map.width]) {
            let cell = table.nodeAt(pos);
            let copy = cell.type.create(util_1.setAttr(cell.attrs, "rowspan", cell.attrs.rowspan - 1), cell.content);
...
function mergeCells(state, dispatch) {
    ...
    if (dispatch) {
        ...
        tr.setNodeMarkup(mergedPos + rect.tableStart, null, util_1.setAttr(util_1.addColSpan(mergedCell.attrs, mergedCell.attrs.colspan, (rect.right - rect.left) - mergedCell.attrs.colspan), "rowspan", rect.bottom - rect.top));
        if (content.size) {
...
function splitCell(state, dispatch) {
    const nodeTypes = schema_1.tableNodeTypes(state.schema);
...
function splitCellWithType(getCellType) {
    return (state, dispatch) => {
        ...
        if (!(sel instanceof cellselection_1.CellSelection)) {
            cellNode = util_1.cellWrapping(sel.$from);
            if (!cellNode)
                return false;
            cellPos = util_1.cellAround(sel.$from).pos;
        }
        else {
        ...
        if (dispatch) {
            ...
            if (baseAttrs.rowspan > 1)
                baseAttrs = util_1.setAttr(baseAttrs, "rowspan", 1);
            if (baseAttrs.colspan > 1)
                baseAttrs = util_1.setAttr(baseAttrs, "colspan", 1);
            let rect = selectedRect(state), tr = state.tr;
            for (let i = 0; i < rect.right - rect.left; i++)
                attrs.push(colwidth ? util_1.setAttr(baseAttrs, "colwidth", colwidth && colwidth[i] ? [colwidth[i]] : null) : baseAttrs);
            let lastCell;
            for (let row = rect.top; row < rect.bottom; row++) {
...
function setCellAttr(name, value) {
    return function (state, dispatch) {
        if (!util_1.isInTable(state))
            return false;
        let $cell = util_1.selectionCell(state);
            ...
            if (state.selection instanceof cellselection_1.CellSelection)
                state.selection.forEachCell((node, pos) => {
                    if (node.attrs[name] !== value)
                        tr.setNodeMarkup(pos, null, util_1.setAttr(node.attrs, name, value));
                });
            else
                tr.setNodeMarkup($cell.pos, null, util_1.setAttr($cell.nodeAfter.attrs, name, value));
            dispatch(tr);
        }
        return true;
    };
...
function deprecated_toggleHeader(type) {
    return function (state, dispatch) {
        if (!util_1.isInTable(state))
            return false;
        if (dispatch) {
            let types = schema_1.tableNodeTypes(state.schema);
            let rect = selectedRect(state), tr = state.tr;
...
function goToNextCell(direction) {
    return function (state, dispatch) {
        if (!util_1.isInTable(state))
            return false;
        let cell = findNextCell(util_1.selectionCell(state), direction);
        if (cell == null)
            return;
        if (dispatch) {
            let $cell = state.doc.resolve(cell);
            dispatch(state.tr.setSelection(prosemirror_state_1.TextSelection.between($cell, util_1.moveCellForward($cell))).scrollIntoView());
        }
        return true;
    };
}
