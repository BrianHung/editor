...
function pastedCells(slice) {
    ...
    if (role == "row") {
        for (let i = 0; i < content.childCount; i++) {
            ...
            if (left || right)
                cells = fitSlice((0, schema_1.tableNodeTypes)(schema).row, new prosemirror_model_1.Slice(cells, left, right)).content;
            rows.push(cells);
        }
    }
    else if (role == "cell" || role == "tableheader") {
        rows.push(openStart || openEnd ? fitSlice((0, schema_1.tableNodeTypes)(schema).row, new prosemirror_model_1.Slice(content, openStart, openEnd)).content : content);
    }
    else {
        return null;
    }
...
function ensureRectangular(schema, rows) {
    ...
    for (let r = 0; r < widths.length; r++) {
        ...
        if (widths[r] < width) {
            let empty = (0, schema_1.tableNodeTypes)(schema).tablecell.createAndFill(), cells = [];
        ...
        for (let row = 0; row < rows.length; row++) {
            ...
            for (let col = added[row] || 0, i = 0; col < newWidth; i++) {
                ...
                if (col + cell.attrs.colspan > newWidth)
                    cell = cell.type.create((0, util_1.removeColSpan)(cell.attrs, cell.attrs.colspan, col + cell.attrs.colspan - newWidth), cell.content);
                cells.push(cell);
        ...
        for (let row = 0, i = 0; row < newHeight; row++, i++) {
            ...
            for (let j = 0; j < source.childCount; j++) {
                ...
                if (row + cell.attrs.rowspan > newHeight)
                    cell = cell.type.create((0, util_1.setAttr)(cell.attrs, "rowspan", Math.max(1, newHeight - cell.attrs.rowspan)), cell.content);
                cells.push(cell);
            }
            newRows.push(prosemirror_model_1.Fragment.from(cells));
        }
...
function growTable(tr, map, table, start, width, height, mapFrom) {
    let schema = tr.doc.type.schema, types = (0, schema_1.tableNodeTypes)(schema), empty, emptyHead;
    if (width > map.width) {
...
function isolateHorizontal(tr, map, table, start, left, right, top, mapFrom) {
    ...
    for (let col = left; col < right; col++) {
        ...
        if (map.map[index - map.width] == pos) {
            ...
            tr.setNodeMarkup(tr.mapping.slice(mapFrom).map(pos + start), null, (0, util_1.setAttr)(cell.attrs, "rowspan", top - cellTop));
            tr.insert(tr.mapping.slice(mapFrom).map(map.positionAt(top, cellLeft, table)), cell.type.createAndFill((0, util_1.setAttr)(cell.attrs, "rowspan", (cellTop + cell.attrs.rowspan) - top)));
...
function isolateVertical(tr, map, table, start, top, bottom, left, mapFrom) {
    ...
    for (let row = top; row < bottom; row++) {
        ...
        if (map.map[index - 1] == pos) {
            ...
            tr.setNodeMarkup(updatePos, null, (0, util_1.removeColSpan)(cell.attrs, left - cellLeft, cell.attrs.colspan - (left - cellLeft)));
            tr.insert(updatePos + cell.nodeSize, cell.type.createAndFill((0, util_1.removeColSpan)(cell.attrs, 0, left - cellLeft)));
