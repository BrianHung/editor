...
const table_nodeview_1 = require("./table-nodeview");
const schema_1 = require("./schema");
exports.key = new prosemirror_state_1.PluginKey("TableColumnResizing");
function columnResizing({ handleWidth = 5, cellMinWidth = 25, View = table_nodeview_1.TableView, lastColumnResizable = true } = {}) {
    let plugin = new prosemirror_state_1.Plugin({
        key: exports.key,
        state: {
            init(_, state) {
                this.spec.props.nodeViews[schema_1.tableNodeTypes(state.schema).table.name] =
...
class ResizeState {
    ...
    apply(tr) {
        ...
        if (state.activeHandle > -1 && tr.docChanged) {
            ...
            if (!util_1.pointsAtCell(tr.doc.resolve(handle)))
                handle = null;
    ...
    if (!pluginState.dragging) {
        ...
        if (cell != pluginState.activeHandle) {
            if (!lastColumnResizable && cell !== -1) {
                ...
                let table = $cell.node(-1), map = tablemap_1.TableMap.get(table), start = $cell.start(-1);
...
function edgeCell(view, event, side) {
    ...
    if (!found)
        return -1;
    let { pos } = found;
    let $cell = util_1.cellAround(view.state.doc.resolve(pos));
    if (!$cell)
        return -1;
    if (side == "right")
        return $cell.pos;
    let map = tablemap_1.TableMap.get($cell.node(-1)), start = $cell.start(-1);
...
function updateColumnWidth(view, cell, width) {
    ...
    for (let row = 0; row < map.height; row++) {
        ...
        tr.setNodeMarkup(start + pos, null, util_1.setAttr(attrs, "colwidth", colwidth));
    }
    if (tr.docChanged)
        view.dispatch(tr);
}
function displayColumnWidth(view, cell, width, cellMinWidth) {
    ...
    let col = tablemap_1.TableMap.get(table).colCount($cell.pos - start) + $cell.nodeAfter.attrs.colspan - 1;
    let dom = view.domAtPos($cell.start(-1)).node;
    while (dom.nodeName != "TABLE")
        dom = dom.parentNode;
    table_nodeview_1.updateColumns(table, dom.firstChild, dom, cellMinWidth, col, width);
}
function zeroes(n) {
    let result = [];
    for (let i = 0; i < n; i++)
