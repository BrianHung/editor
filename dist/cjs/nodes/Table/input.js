...
exports.handleKeyDown = prosemirror_keymap_1.keydownHandler({
...
function arrow(axis, dir) {
    return (state, dispatch, view) => {
        ...
        if (axis == "horiz") {
            ...
            let $cell = state.doc.resolve(end), $next = util_1.nextCell($cell, axis, dir), newSel;
            if ($next)
...
function shiftArrow(axis, dir) {
    return (state, dispatch, view) => {
        ...
        let $head = util_1.nextCell(sel.$headCell, axis, dir);
        if (!$head)
            return false;
...
function deleteCellSelection(state, dispatch) {
    ...
    if (!(sel instanceof cellselection_1.CellSelection))
        return false;
    if (dispatch) {
        let tr = state.tr, baseContent = schema_1.tableNodeTypes(state.schema).tablecell.createAndFill().content;
...
function handleTripleClick(view, pos) {
    let doc = view.state.doc, $cell = util_1.cellAround(doc.resolve(pos));
    if (!$cell)
        return false;
...
function handlePaste(view, _, slice) {
    if (!util_1.isInTable(view.state))
        return false;
    let cells = copypaste_1.pastedCells(slice), sel = view.state.selection;
    if (sel instanceof cellselection_1.CellSelection) {
        if (!cells)
            cells = { width: 1, height: 1, rows: [prosemirror_model_1.Fragment.from(copypaste_1.fitSlice(schema_1.tableNodeTypes(view.state.schema).tablecell, slice))] };
        ...
        cells = copypaste_1.clipCells(cells, rect.right - rect.left, rect.bottom - rect.top);
        copypaste_1.insertCells(view.state, view.dispatch, start, rect, cells);
        return true;
    }
    else if (cells) {
        let $cell = util_1.selectionCell(view.state), start = $cell.start(-1);
        copypaste_1.insertCells(view.state, view.dispatch, start, tablemap_1.TableMap.get($cell.node(-1)).findCell($cell.pos - start), cells);
        return true;
    }
    else {
        return false;
...
function handleMouseDown(view, startEvent) {
    ...
    if (startEvent.shiftKey && (view.state.selection instanceof cellselection_1.CellSelection)) {
        ...
        ($anchor = util_1.cellAround(view.state.selection.$anchor)) != null &&
    ...
    function setCellSelection($anchor, event) {
        ...
        if (!$head || !util_1.inSameTable($anchor, $head)) {
            if (starting)
                $head = $anchor;
            else
                return;
...
function cellUnderMouse(view, event) {
    ...
    if (!mousePos)
        return null;
    return mousePos ? util_1.cellAround(view.state.doc.resolve(mousePos.pos)) : null;
}
