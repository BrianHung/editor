...
function fixTable(state, table, tablePos, tr) {
    ...
    for (let i = 0; i < map.problems.length; i++) {
        ...
        if (prob.type == "collision") {
            ...
            tr.setNodeMarkup(tr.mapping.map(tablePos + 1 + prob.pos), null, util_1.removeColSpan(cell.attrs, cell.attrs.colspan - prob.n, prob.n));
        }
            ...
            let cell = table.nodeAt(prob.pos);
            tr.setNodeMarkup(tr.mapping.map(tablePos + 1 + prob.pos), null, util_1.setAttr(cell.attrs, "rowspan", cell.attrs.rowspan - prob.n));
        }
        else if (prob.type == "colwidth mismatch") {
            let cell = table.nodeAt(prob.pos);
            tr.setNodeMarkup(tr.mapping.map(tablePos + 1 + prob.pos), null, util_1.setAttr(cell.attrs, "colwidth", prob.colwidth));
        }
    }
    let first, last;
    ...
    for (let i = 0, pos = tablePos + 1; i < map.height; i++) {
        ...
        if (add > 0) {
            ...
            let nodes = [];
            for (let j = 0; j < add; j++)
                nodes.push(schema_1.tableNodeTypes(state.schema)[tableNodeType].createAndFill());
