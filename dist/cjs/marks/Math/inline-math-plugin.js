...
function reduceMathDecorations(decorations, match) {
    try {
        const html = (0, katex_1.renderToString)(match.match);
        const toDOM = () => { const span = document.createElement("span"); span.innerHTML = html; return span; };
        decorations.push(prosemirror_view_1.Decoration.widget(match.from + 1, toDOM));
    }
    catch (error) {
    }
    decorations.push(prosemirror_view_1.Decoration.inline(match.from + 1, match.to + 1, { class: "math-content" }));
    return decorations;
}
...
function InlineMathDecorationsPlugin() {
    const pluginKey = new prosemirror_state_1.PluginKey("InlineMathPlugin");
    return new prosemirror_state_1.Plugin({
        ...
        state: {
            init(config, state) {
                ...
                return { modified: [], decorations: prosemirror_view_1.DecorationSet.create(state.doc, decorations), lastTransaction: null };
            },
            apply(tr, prevState) {
        ...
        appendTransaction(transactions, oldState, newState) {
            ...
            const textInsert = transactions.some(({ steps }) => steps.some((step) => step instanceof prosemirror_transform_1.ReplaceStep || step instanceof prosemirror_transform_1.ReplaceAroundStep));
...
exports.default = InlineMathDecorationsPlugin;
