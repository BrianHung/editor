...
import { addColSpan, cellAround, cellWrapping, columnIsHeader, isInTable, moveCellForward, removeColSpan, selectionCell, setAttr } from "./util";
...
export function addColumn(tr, { map, tableStart, table }, col) {
    ...
    for (let row = 0; row < map.height; row++) {
        ...
        if (col > 0 && col < map.width && map.map[index - 1] == map.map[index]) {
            ...
            let type = refColumn == null ? tableNodeTypes(table.type.schema).tablecell
...
export function addRow(tr, { map, tableStart, table }, row) {
    ...
    for (let col = 0, index = map.width * row; col < map.width; col++, index++) {
        if (row > 0 && row < map.height && map.map[index] == map.map[index - map.width]) {
            ...
            let type = refRow == null ? tableNodeTypes(table.type.schema).tablecell
...
function deprecated_toggleHeader(type) {
    return function (state, dispatch) {
        ...
        if (dispatch) {
            ...
            let rect = selectedRect(state), tr = state.tr;
            let cells = rect.map.tablecellsInRect(type == "column" ? new Rect(rect.left, 0, rect.right, rect.map.height) :
